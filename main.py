...
def is_channel_allowed(channel_id: str) -> bool:
    ...
    Returns True if SLACK_ALLOWED_CHANNELS is empty (allow all) or if the
    channel_id is in the allowed list.
    """
    if not config.SLACK_ALLOWED_CHANNELS:
        return True  # Empty = allow all channels
    
    allowed = [ch.strip() for ch in config.SLACK_ALLOWED_CHANNELS.split(",")]
...
async def handle_reaction_added(event: dict):
    ...
    if not is_channel_allowed(channel_id):
        logger.info(f"Ignoring reaction in non-allowed channel {channel_id}")
        return
    if not info:
